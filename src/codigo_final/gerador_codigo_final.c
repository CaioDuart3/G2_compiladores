...
static void gerar_print(NoAST *expr, FILE *saida, int indent) {
    ...
    char format_str[EXPR_BUFFER_SIZE] = "";
    char args_str[EXPR_BUFFER_SIZE] = "";
    int primeira = 1;
    
    while (args) {
        if (args->tipo == NO_STRING) {
            strcat(format_str, args->valor_string);
        } else {
            ...
            if (!primeira) strcat(args_str, ", ");
            strcat(args_str, expr_c);
            primeira = 0;
        }
            ...
            if (iter && iter->tipo == NO_CHAMADA_FUNCAO) {
                ...
                if (func_id && func_id->tipo == NO_ID && strcmp(func_id->valor_string, "range") == 0) {
                    ...
                    if (args) {
                        ...
                        fprintf(saida, "for (int %s = %s; %s < %s; %s++) {\n", var->valor_string, start, var->valor_string, end, var->valor_string);
                        gerar_bloco(no->filho3, saida, indent + 1);
                        indenta(saida, indent);
                        fprintf(saida, "}\n");
                    }
